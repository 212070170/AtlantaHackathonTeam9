...
var deployment_vars = {
    server_port:80,
    api_port:8080,
    api_port_secondary:8001
}

process.argv.forEach(function (val, index, array) {
    if(val.toUpperCase() == "DEV"){
        deployment_vars = {
            server_port:3000,
            api_port:8002,
            api_port_secondary:8001
...
var config = {
	express: {
		port: process.env.VCAP_APP_PORT || deployment_vars.server_port
	}
};
...
app.use('/sop',express.static(process.env.HOME+'/documents'));

console.log(process.env.HOME+'/documents');
...
app.use(proxyMiddleware("/api",{
        port:'8001',
    target:"http://127.0.0.1:"+deployment_vars.api_port,
    changeOrigin:true,
    logLevel: 'debug',
    pathRewrite: { '^/api/services/*': 'http://127.0.0.1:'+deployment_vars.api_port+'/services/'},
...
app.use(proxyMiddleware("/feedback",{
        port:'8001',
        target:"http://127.0.0.1:"+deployment_vars.api_port_secondary,
        changeOrigin:true,
        logLevel: 'debug',
        pathRewrite: { '^/feedback/services/*': 'http://127.0.0.1:'+deployment_vars.api_port_secondary+'/services/'},
        onProxyReq: function onProxyReq(proxyReq, req, res) {
            req.headers['Authorization'] = auth.getUserToken(req);
            req.headers['Content-Type'] = 'application/json';
//            console.log('Request headers: ' + JSON.stringify(req.headers));
